...
var Game = /** @class */ (function (_super) {
            ...
            for (i = 0; i < sizeX - 1; i++) {
                for (j = 0; j < sizeY - 1; j++) {
                    map[i][j] = (map[i][j] +
                        ...
                        map[i][j + 1] +
                        map[i + 1][j + 1]) / 4;
                    map[i][j] = (map[(sizeX - 1) - i][(sizeY - 1) - j] +
                        map[(sizeX - 1) - i - 1][(sizeY - 1) - j] +
                        map[(sizeX - 1) - i][(sizeY - 1) - j - 1] +
                        map[(sizeX - 1) - i - 1][(sizeY - 1) - j - 1]) / 4;
                }
            }
        }
            ...
            for (i = 0; i < sizeX; i++) {
                for (j = 0; j < sizeY; j++) {
                    ...
                    if (map[i][j] === tileCurrent) {
                        // Left up
                        if (i > 0 && j > 0 && map[i - 1][j - 1] !== tileCurrent && map[i - 1][j - 1] !== tileAbove && map[i - 1][j - 1] !== tileBelow) {
                            map[i - 1][j - 1] = tileBelow;
                        }
                        // Mid up
                        if (j > 0 && map[i][j - 1] !== tileCurrent && map[i][j - 1] !== tileAbove && map[i][j - 1] !== tileBelow) {
                            map[i][j - 1] = tileBelow;
                        }
                        // Right up
                        if (i < sizeX - 1 && j > 0 && map[i + 1][j - 1] !== tileCurrent && map[i + 1][j - 1] !== tileAbove && map[i + 1][j - 1] !== tileBelow) {
                            map[i + 1][j - 1] = tileBelow;
                        }
                        // Right mid
                        if (i < sizeX - 1 && map[i + 1][j] !== tileCurrent && map[i + 1][j] !== tileAbove && map[i + 1][j] !== tileBelow) {
                            map[i + 1][j] = tileBelow;
                        }
                        // Right down
                        if (i < sizeX - 1 && j < sizeY - 1 && map[i + 1][j + 1] !== tileCurrent && map[i + 1][j + 1] !== tileAbove && map[i + 1][j + 1] !== tileBelow) {
                            map[i + 1][j + 1] = tileBelow;
                        }
                        // Mid down
                        if (j < sizeY - 1 && map[i][j + 1] !== tileCurrent && map[i][j + 1] !== tileAbove && map[i][j + 1] !== tileBelow) {
                            map[i][j + 1] = tileBelow;
                        }
                        // Left down
                        if (i > 0 && j < sizeY - 1 && map[i - 1][j + 1] !== tileCurrent && map[i - 1][j + 1] !== tileAbove && map[i - 1][j + 1] !== tileBelow) {
                            map[i - 1][j + 1] = tileBelow;
                        }
                        // Left mid
                        if (i > 0 && map[i - 1][j] !== tileCurrent && map[i - 1][j] !== tileAbove && map[i - 1][j] !== tileBelow) {
                            map[i - 1][j] = tileBelow;
                        }
                    }
                }
    ...
    Game.prototype.statePreplay2Play = function () {
    ...
    Game.prototype.statePlay2Postplay = function () {
    ...
    Game.prototype.updateEnemySpawn = function () {
        var enemy, i;
    ...
    Game.prototype.updateEnemySpawnGround = function () {
    ...
    Game.prototype.updateCloudSpawn = function () {
        var cloud;
    ...
    Game.prototype.bulletVSmob = function (bullet, mob) {
        bullet.kill();
    ...
    Game.prototype.playerVSmob = function (player, mob) {
        mob.kill();
        this.explode(mob);
        this.playerVSenemy(player);
    };
    Game.prototype.playerVSbullet = function (player, bullet) {
        bullet.kill();
        this.playerVSenemy(player);
    };
    Game.prototype.playerVSenemy = function (player) {
    ...
    Game.prototype.playerVSbonus = function (player, bonus) {
        bonus.kill();
        player.collectUpgrade(bonus.bonusClass);
        this.updateGUI();
    };
    // TODO : mob method
    Game.prototype.explode = function (thing) {
    ...
    Game.prototype.updateBackground = function (delta) {
        // SCROLLING
    ...
    Game.prototype.render = function () {
        ...
        if (CONFIG.DEBUG.bottomInfos) {
            this.game.debug.text('ground.y : ' + Math.round(this.ground.y) + 'px | ' +
                ...
                (100 - this.player.bulletPool.countDead()) + ' bullets | ', 0, CONFIG.GAME_HEIGHT * CONFIG.PIXEL_RATIO - 16);
            this.game.debug.text(
            // 'player.health : ' + this.player.health + ' | ' +
            'Camera position : ' + this.camera.x + '/' + this.camera.y + ' | ' +
                'SCROLL : ' + Math.round(this.ground.y % (28 * CONFIG.PIXEL_RATIO)), 0, CONFIG.GAME_HEIGHT * CONFIG.PIXEL_RATIO - 16 + 16);
        }
    };
    return Game;
}(Phaser.State));
